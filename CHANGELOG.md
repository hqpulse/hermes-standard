...
- **Humanizer skill.** `skills/humanizer/` is blader/humanizer (MIT, license
  ...
  someone else: a mail draft, a note that will be shared, a document.
...
- **Presets.** `skills/assistant-standard/presets/`: `morning-brief.json`
  ...
  it; the skill says how keep, change and stop are handled in chat. The
  mechanics and what the controller must allow are in `presets/README.md`.
...
- Every new file is listed in `distribution_owned`: with an explicit list, only
  listed paths reach the pod at all.
