...
- When a message needs no words back (a thanks, an FYI, good news, a photo shared for you to see), react to it with one fitting emoji instead of replying. Never react to a question or a request; those get an answer. One emoji, chosen for that message.
